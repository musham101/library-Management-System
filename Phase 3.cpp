...
using namespace std;
 ...
 const int MAX_BOOK_ID = 13, DATE_SIZE = 11, MAX_COPY_NUMBER = 17, MAX_BOOKS = 200, MAX_CAEGORIES = 30, MAX_BOOK_NAME = 60, MAX_CATEGORY_NAME = 20, MAX_CATEGORY_NUMBER = 4, MAX_STUDENTS = 100, MAX_REGISTRATION_NUMBER = 12, MAX_STUDENT_NAME = 60, NUMBER_OF_USERS = 1, MAX_ISSUe_SIZE = 50;
 ...
 void studentCurrentBook(BookIssue arr[], int size, char regNo[]){
 	int s, d = 0;
 	cout << "\tCopy Id\t\t\tIssued Date\n";
	for(s = 0; s < x4; s++){
		while(d < MAX_REGISTRATION_NUMBER){
			if(regNo[d] == arr[s].regNo[d] and regNo[d + 1] == arr[s].regNo[d + 1] and regNo[d + 2] == arr[s].regNo[d + 2] and regNo[d + 3] == arr[s].regNo[d + 3] and regNo[d + 4] == arr[s].regNo[d + 4] and regNo[d + 5] == arr[s].regNo[d + 5] and regNo[d + 6] == arr[s].regNo[d + 6] and regNo[d + 7] == arr[s].regNo[d + 7] and regNo[d + 8] == arr[s].regNo[d + 8] and regNo[d + 9] == arr[s].regNo[d + 9] and regNo[d + 10] == arr[s].regNo[d + 10]){
		    	cout << "\t" << arr[s].bookCopyId << "\t" << arr[s].issueDtae.Day << "-" << arr[s].issueDtae.Month << "-" << arr[s].issueDtae.Year;
		    	break;
			}
		}
		cout << endl;
 ...
 void checkBookStatus(BookIssue arr1[], BookCopy arr[], int size, int bookCopySize , char copyId[]){
 	int s, d, flag;
	for(s = 0; s < x1; s++){
		while(d < MAX_COPY_NUMBER){
			if(copyId[d] == arr[s].Id[d] and copyId[d + 1] == arr[s].Id[d + 1] and copyId[d + 2] == arr[s].Id[d + 2] and copyId[d + 3] == arr[s].Id[d + 3] and copyId[d + 4] == arr[s].Id[d + 4] and copyId[d + 5] == arr[s].Id[d + 5] and copyId[d + 6] == arr[s].Id[d + 6] and copyId[d + 7] == arr[s].Id[d + 7] and copyId[d + 8] == arr[s].Id[d + 8] and copyId[d + 9] == arr[s].Id[d + 9] and copyId[d + 10] == arr[s].Id[d + 10] and copyId[d + 11] == arr[s].Id[d + 11] and copyId[d + 12] == arr[s].Id[d + 12] and copyId[d + 13] == arr[s].Id[d + 13] and copyId[d + 14] == arr[s].Id[d + 14] and copyId[d + 15] == arr[s].Id[d + 15]){
				flag = 1;
				break;
			}
			else{
	...
	if(flag == 0){
		cout << "Book not availble\n";
	}
	else{
		for(s = 0; s < x1; s++){
			d = 0;
    		while(d < MAX_COPY_NUMBER){
    			if(copyId[d] == arr1[s].bookCopyId[d] and copyId[d + 1] == arr1[s].bookCopyId[d + 1] and copyId[d + 2] == arr1[s].bookCopyId[d + 2] and copyId[d + 3] == arr1[s].bookCopyId[d + 3] and copyId[d + 4] == arr1[s].bookCopyId[d + 4] and copyId[d + 5] == arr1[s].bookCopyId[d + 5] and copyId[d + 6] == arr1[s].bookCopyId[d + 6] and copyId[d + 7] == arr1[s].bookCopyId[d + 7] and copyId[d + 8] == arr1[s].bookCopyId[d + 8] and copyId[d + 9] == arr1[s].bookCopyId[d + 9] and copyId[d + 10] == arr1[s].bookCopyId[d + 10] and copyId[d + 11] == arr1[s].bookCopyId[d + 11] and copyId[d + 12] == arr1[s].bookCopyId[d + 12] and copyId[d + 13] == arr1[s].bookCopyId[d + 13] and copyId[d + 14] == arr1[s].bookCopyId[d + 14] and copyId[d + 15] == arr1[s].bookCopyId[d + 15]){
    				flag = 1;
    				break;
    			}
    			else{
    	...
    	if(flag == 0){
    		...
    		if(arr1[s].status == issued){
    			cout << "The book is currently issued to the student " << arr1[s].regNo << ".\n";
    			cout << "Expected return date is " << arr1[s].issueDtae.Day + 14 << "-" << arr1[s].issueDtae.Month << "-" << arr1[s].issueDtae.Year << endl;
			}
		}
	}
 ...
 int returnBook(BookIssue arr[], int size, char copyIdtoReturn[], Date date){
 	int s, d = 0, flag, diff;
	for(s = 0; s < x4; s++){
		while(d < MAX_COPY_NUMBER){
			if(copyIdtoReturn[d] == arr[s].bookCopyId[d] and copyIdtoReturn[d + 1] == arr[s].bookCopyId[d + 1] and copyIdtoReturn[d + 2] == arr[s].bookCopyId[d + 2] and copyIdtoReturn[d + 3] == arr[s].bookCopyId[d + 3] and copyIdtoReturn[d + 4] == arr[s].bookCopyId[d + 4] and copyIdtoReturn[d + 5] == arr[s].bookCopyId[d + 5] and copyIdtoReturn[d + 6] == arr[s].bookCopyId[d + 6] and copyIdtoReturn[d + 7] == arr[s].bookCopyId[d + 7] and copyIdtoReturn[d + 8] == arr[s].bookCopyId[d + 8] and copyIdtoReturn[d + 9] == arr[s].bookCopyId[d + 9] and copyIdtoReturn[d + 10] == arr[s].bookCopyId[d + 10] and copyIdtoReturn[d + 11] == arr[s].bookCopyId[d + 11] and copyIdtoReturn[d + 12] == arr[s].bookCopyId[d + 12] and copyIdtoReturn[d + 13] == arr[s].bookCopyId[d + 13] and copyIdtoReturn[d + 14] == arr[s].bookCopyId[d + 14] and copyIdtoReturn[d + 15] == arr[s].bookCopyId[d + 15]){
				flag = 1;
				break;
			}
			else{
	...
	if(flag == 0){
		...
		if(arr[s].status == issued){
			diff = date.Day - arr[s].issueDtae.Day;
			if(diff <= 14){
				cout << "Student " << arr[s].regNo << " has returned the book with zero fine\n";
			}
			else{
				cout << "Student " << arr[s].regNo << " has returned the book with late return. Student has been fined PKR " << ((diff - 14) * 10) << endl;
			}
		}
	}
 ...
 bool issueBook(BookIssue arr[], int size, BookIssue toIssue){
    ...
    for(d = 0; d < MAX_REGISTRATION_NUMBER; d++){
    	arr[s].regNo[d] = toIssue.regNo[d];
    }
    for(d = 0; d < MAX_COPY_NUMBER; d++){
    	arr[s].bookCopyId[d] = toIssue.bookCopyId[d];
    }
    arr[s].status = issued;
    arr[s].issueDtae.Day = toIssue.issueDtae.Day;
    arr[s].issueDtae.Month = toIssue.issueDtae.Month;
    arr[s].issueDtae.Year = toIssue.issueDtae.Year;
    s++;
    x4++;
    
 }
 
 bool user(char a[], char b[]){
    int s, flag, flag1;
	char ur[19], ps[12];
	ifstream fin;
	fin.open("users.txt");
 ...
 void TakeMenuInput(char& mainOption, char& subOption){
	...
	if(mainOption == '1'){
		...
		if(subOption == 'I'){
			...
			cin >> r.bookCopyId;
			cin >> r.issueDtae.Day;
			cin.ignore(r.issueDtae.Day, '-');
			cin >> r.issueDtae.Month;
			cin.ignore(r.issueDtae.Month, '-');
			cin >> r.issueDtae.Year;
			resp = isValidRegistrationNo(r.regNo);
			resp1 = isValidCopyNumber(r.bookCopyId);
			if(resp == true and resp1 == true){
				resp = issueBook(x4 ,MAX_ISSUe_SIZE, r);
				if(resp == true){
					cout << "Book has been issued to " << r.regNo << endl;;
				}
				
			}
			else{
				cout << "You entered invalid RegNo or Book Copy Id\n";
			}
		}
		else if(subOption == 'R'){
			...
			resp1 = isValidDate(n);
			if(resp == true and resp1 == true){
				returnBook(x4, MAX_ISSUe_SIZE, a, n);
			}
			else{
			...
			cout << "Enter the Copy Id of thr book: ";
			cin >> a;
			resp = isValidCopyNumber(a);
			if(resp == true){
				checkBookStatus(x4, x2, MAX_ISSUe_SIZE, MAX_COPY_NUMBER, a);
			}
			else{
				cout << "You entered invalid entry\n";
			}	
			...
			resp = isValidRegistrationNo(a);
			if(resp == true){
				studentCurrentBook(x4, MAX_ISSUe_SIZE, a);
			}
		}
	}
	else{
 ...
 void printMenu(){
	...
	while(true){
    	...
    	if(resp == true){
    		...
    		cout << "You entered invalid username or password\n";
    	}
	}
